...
class Events(commands.Cog):
    ...
    async def on_command_error(self, ctx, err):
        if isinstance(err, errors.MissingRequiredArgument) or isinstance(err, errors.BadArgument):
            ...
            await ctx.send(f":flag_br: Houve um erro ao executar este comando! \n :flag_us: There was an error on executing this command!")
            ...
            await ctx.send(f"Aguarde para executar este comando novamente!  {err.retry_after:.2f} segundos.")
    ...
    async def on_command(self, ctx):
        try:
          print(f"{ctx.guild.name} > {ctx.author} > {ctx.message.clean_content}")
            ...
            print(f"Mensagem Privada > {ctx.author} > {ctx.message.clean_content}")
