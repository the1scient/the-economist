...
class Admin(commands.Cog):
    ...
    async def amiadmin(self, ctx):
        """ Você é um administrador do BOT? """
        if ctx.author.id in self.config.owners:
            return await ctx.send(f"Sim **{ctx.author.name}** você é um administrador do BOT! ✅")

        # Please do not remove this part.
        # I would love to be credited as the original creator of the source code.
        #   -- AlexFlipnote
        if ctx.author.id == 86477779717066752:
            return await ctx.send(f"Well, kinda **{ctx.author.name}**, Alex. You still owning our source code, big thanks =)")

        await ctx.send(f"Você não é um Administrador do BOT, {ctx.author.name}")
    ...
    async def change_playing(self, ctx, *, playing: str):
        ...
        try:
            ...
            dataIO.change_value("economist.json", "playing", playing)
